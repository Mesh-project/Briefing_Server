...
from api.models import model_analysis, model_user
...
class SignIn(View):
    def post(self, request):
        ...
        try:
            if model_user.objects.filter(user_id=data["user_id"]).exists():
                user = model_user.objects.get(user_id=data["user_id"])

                if bcrypt.checkpw(data['user_pw'].encode('UTF-8'), user.user_pw.encode('UTF-8')):
                    token = jwt.encode({'user_id' : user.user_id}, SECRET_KEY, algorithm='HS256').decode('UTF-8')
                    return JsonResponse({"token": token}, status=200)
                return JsonResponse({'message' : "Wrong Password"}, status=401)
            return JsonResponse({'message' : "No ID"},status=400)
        except KeyError:
...
# 아래코드들은 예시로 적어둔 api로 참고용으로만 사용후 수정 예정
...
def user_list(request):
    if request.method == 'GET':
        query_set = model_user.objects.all()
